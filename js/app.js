...
function playApproveStamp(done) {
  const fx = $('approve-fx');
  fx.classList.remove('hide', 'fade', 'on');
  void fx.offsetWidth;            // 리플로우로 애니메이션 재시작 보장
  fx.classList.add('on');
  setTimeout(() => {
    done();                       // 도장이 찍힌 뒤 결과 렌더 + 스크롤
    fx.classList.add('fade');     // 오버레이는 부드럽게 사라짐
    setTimeout(() => fx.classList.add('hide'), 240);
  }, 520);
}
